...
final class Decent_Lightbox_Admin {
	...
	public function add_attachment_field( array $form_fields, WP_Post $post ): array {
		...
		$checkbox .= esc_html__( 'Open this image in the lightbox on click', 'seo-wunderkiste' );
		$checkbox .= '</label>';

		$form_fields[ DECENT_LIGHTBOX_META_KEY ] = array(
			'label' => __( 'Lightbox', 'seo-wunderkiste' ),
			'input' => 'html',
			'html'  => $checkbox,
			'helps' => __( 'When enabled, clicking the image on the front end opens it in a lightweight overlay.', 'seo-wunderkiste' ),
		);
	...
	public function add_media_column( array $columns ): array {
		$columns['decent_lightbox'] = __( 'Lightbox', 'seo-wunderkiste' );
		return $columns;
	}
	...
	public function render_media_column( string $column_name, int $post_id ): void {
		...
		if ( Decent_Lightbox::is_enabled_for_attachment( $post_id ) ) {
			...
			echo '<span class="screen-reader-text">' . esc_html__( 'Lightbox enabled', 'seo-wunderkiste' ) . '</span>';
		} else {
			echo '<span aria-hidden="true">—</span>';
			echo '<span class="screen-reader-text">' . esc_html__( 'Lightbox disabled', 'seo-wunderkiste' ) . '</span>';
		}
	}
}
