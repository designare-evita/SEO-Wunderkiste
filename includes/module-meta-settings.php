...
function seowk_meta_add_meta_box() {
    foreach ( array( 'post', 'page' ) as $screen ) {
        add_meta_box( 'seowk_meta_box', __( '🔍 SEO Meta Einstellungen', 'seo-wunderkiste' ), 'seowk_meta_render_meta_box', $screen, 'normal', 'high' );
    }
}
...
function seowk_meta_render_meta_box( $post ) {
    ...
    <div class="seowk-meta-box" style="padding: 15px 0;">
        ...
        <p><label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php esc_html_e( 'SEO Title (max. 60 Zeichen)', 'seo-wunderkiste' ); ?></label>
        <input type="text" name="seowk_meta_title" value="<?php echo esc_attr( $meta_title ); ?>" class="large-text" maxlength="70" /></p>
        <p><label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php esc_html_e( 'Meta Description (max. 160 Zeichen)', 'seo-wunderkiste' ); ?></label>
        ...
        <p><label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php esc_html_e( 'Robots Meta Tag', 'seo-wunderkiste' ); ?></label>
        ...
        <p><label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php esc_html_e( 'OG Title', 'seo-wunderkiste' ); ?></label>
        <input type="text" name="seowk_og_title" value="<?php echo esc_attr( $og_title ); ?>" class="large-text" /></p>
        <p><label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php esc_html_e( 'OG Description', 'seo-wunderkiste' ); ?></label>
        <textarea name="seowk_og_description" rows="2" class="large-text"><?php echo esc_textarea( $og_description ); ?></textarea></p>
        <p><label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php esc_html_e( 'OG Image URL', 'seo-wunderkiste' ); ?></label>
...
function seowk_meta_output_head() {
    ...
    // A NoIndex flag from the Bulk NoIndex module always wins over the field.
    if ( get_post_meta( $post_id, '_seowk_noindex', true ) ) {
        $meta_robots = 'noindex, nofollow';
    }
    if ( ! empty( $meta_robots ) ) { echo '<meta name="robots" content="' . esc_attr( $meta_robots ) . '">' . "\n"; }
